...
enum SonyHeadphoneModel: String, Equatable, Sendable {
    ...
    case wh1000xm6 = "WH-1000XM6"
    case unknown = "Sony Headphones"

    static func infer(fromDeviceName name: String) -> SonyHeadphoneModel {
        let compact = name.lowercased().replacingOccurrences(
            of: "[^a-z0-9]", with: "", options: .regularExpression
        )
        if compact.contains("wh1000xm6") || compact.contains("xm6") { return .wh1000xm6 }
        if compact.contains("wh1000xm5") || compact.contains("xm5") { return .wh1000xm5 }
        return .unknown
    }
    ...
    var displayLabel: String {
        ...
        case .unknown: return "Unknown Sony Model"
        }
    }
}
...
struct FeatureSupport: Equatable, Sendable {
    ...
    static let xm5Native = FeatureSupport(
        noiseControl: .supported,
        ambientLevel: .supported,
        focusOnVoice: .unsupported(reason: "Focus on Voice is not supported on WH-1000XM5."),
        volume: .supported,
        dseeExtreme: .supported,
        equalizer: .supported,
        speakToChat: .supported,
